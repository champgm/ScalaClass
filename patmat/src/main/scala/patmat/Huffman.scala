...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = runLengthEncoding[Char](chars)
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    ...
    until(leaves => singleton(leaves), leaves => combine(leaves))(sortedLeaves).head
  }
