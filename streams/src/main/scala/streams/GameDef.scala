...
/** This trait represents the layout and building blocks of the game
  *
  * @TODO: SHOULD RENAME `Row` and `Col` in class Pos to `row` and `col`. It's
  * confusing to have `Row` being the vertical axis.
  */
trait GameDef {
  ...
  sealed abstract class Move {
    def go(block: Block): Block
  }
  case object Left extends Move {
    def go(block: Block): Block = block.left
  }
  case object Right extends Move {
    def go(block: Block): Block = block.right
  }
  case object Up extends Move {
    def go(block: Block): Block = block.up
  }
  case object Down extends Move {
    def go(block: Block): Block = block.down
  }
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def legalNeighbors: List[(Block, Move)] = {
      neighbors.filter(item => item._1.isLegal)
