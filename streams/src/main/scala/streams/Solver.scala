...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    val nextMoves = b.legalNeighbors
    (for { (block, move) <- nextMoves } yield (block, move :: history))
      .toStream
  }
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
    if (initial.isEmpty) Stream.Empty
      ...
      val morePaths = for {
        path <- initial
        nextPath <- newNeighborsOnly(neighborsWithHistory(path._1, path._2), explored)
      } yield nextPath
      val newlyExplored = morePaths.map(_._1).toSet
      initial #::: from(morePaths, explored ++ newlyExplored)
    }
  }
